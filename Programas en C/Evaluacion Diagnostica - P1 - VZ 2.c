...
//Lote 1 (de todo:modelo, piezas, stock)--> (i=1,20)(j=1,100) Matriz composici�n --> MC (Modelo, Parte) = Cantidad a Utilizar
	...
	for(i=0;i<cantidadModelo;i++){
		for (j=0; j<cantidadPieza;j++){
			...
			scanf("%i", &piezasPorModelo);
			MC[i][j]=piezasPorModelo;
	}
	printf("\n");
	}
	...
	printf("Si la carga fue correcta ingrese el valor 2, si fue incorrecta ingrese el valor 0...\n");
	scanf("%i", &carga);
	}
...
//Ingreso de la cantidad de piezas en stock
	...
	for(i=0;i<cantidadPieza;i++){
		...
		printf("El stock por modelo es: \n");
		for(i=0;i<cantidadPieza;i++){
			printf("\n Stock del modelo n%i: ", VSIP[i]);
		}
		
	printf("Si la carga fue correcta ingrese el valor 4, si fue incorrecta ingrese el valor 0...\n");
	scanf("%i", &carga);
	}
...
//Proceso	
    ...
    if(cantidadPedida<=VSFM[modelo+1]){
    	printf("Control stock antes del pedido %i, cantidad pedida %i:\n", VSFM[modelo+1], cantidadPedida);
    	VSFM[modelo+1]=VSFM[modelo+1]-cantidadPedida; 
    	...
    	printf("Del modelo numero %i, se pidieron % cantidad de autos, de los cuales habia un stock inicial de %, dejando un stock de %i, se deben fabricar %i y se adeudan %i", modelo+1, cantidadPedida, VSIM[modelo+1], VSFM[modelo+1], AFabricar, ADeuda);
	}
